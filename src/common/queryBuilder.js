 ...
 * a query-builder function (query string → ES 8 DSL bool object).
 *
 * @param {object} kwargs Object of keyword arguments
 ...
 * @param {string} [kwargs.operator] Search operator
 * @returns {Function} Query builder: (queryString: string) => ES bool query object
 */
export function buildQuery({ analyzers, fields, operator }) {
    /**
    ...
    return (query) => {
        ...
        // separate query into inclusions and exclusions based on hyphen indicator
        if (query.match(/-/g)) {
        ...
        const includedQueries = analyzers.flatMap((analyzer) =>
            buildMatchQueries({
                analyzer,
                fields,
        ...
        const excludedQueries = analyzers.flatMap((analyzer) =>
            buildMatchQueries({
                analyzer,
                fields,
                operator: "or",
